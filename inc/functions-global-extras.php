		...
		if( ! $has_migrated ) {
			...
			foreach( $form_obj as $item ) {
				...
				if( strpos( $name, '_product_extra_groups' ) !== false ) {
					...
					if( isset( $name_array[2] ) && isset( $name_array[3] ) && $name_array[2] == 'all' && $name_array[3] == 'is_selected' ) {
						...
						if( ! in_array( $param, $fields_with_multiples ) && strpos( $param, 'condition' ) === false ) {
						...
						} else if( $param == 'child_categories' ) {

							if( isset( $child_categories[$field_id] ) ) {
								$child_categories[$field_id][] = $value;
							} else {
								$child_categories[$field_id] = array( $value );
							}
							...
							if( isset( $weekdays[$field_id] ) ) {
								$weekdays[$field_id][$name_array[2]] = $value;
							} else {
			...
			if( $child_categories ) {
				foreach( $child_categories as $key=>$value ) {
					update_post_meta( $key, 'child_categories', $value );
				}
			}
			if( $condition_fields ) {
...
function pewc_filter_product_extra_groups( $product_extra_groups, $product_id ) {
	...
	if( ! $has_migrated ) {
		...
		$global_order = pewc_get_global_group_order();
	...
	if( $globals ) {
		...
		foreach( $globals as $global_id=>$global ) {
			...
			// Check if any global groups have been added at product-level
			$product = wc_get_product( $product_id );
			if( is_object( $product ) ) {
				$pewc_global_groups_by_product = $product->get_meta( 'pewc_global_groups_by_product' );
				if( $pewc_global_groups_by_product ) {
					$pewc_global_groups_by_product = explode( ',', $pewc_global_groups_by_product );
					foreach( $pewc_global_groups_by_product as $global_id ) {
						$product_extra_groups[$global_id]['items'] = pewc_get_group_fields( $global_id );
					}
				}
			}
...
/**
 * Get the list of global groups in order
 * @since 3.11.2
 */
function pewc_get_global_group_order() {
	$global_order = get_option( 'pewc_global_group_order', '' );
	return apply_filters( 'pewc_global_group_order', $global_order );
}
...
function pewc_get_all_product_ids() {
	...
	$products = pewc_get_transient( 'pewc_product_ids' );
	if( ! $products ) {

		$args = array(
			'numberposts'		=> apply_filters( 'pewc_get_all_product_ids_limit', 9999 ),
		 	'status'			=> apply_filters( 'pewc_get_all_product_ids_status', array( 'publish', 'private' ) ),
			'return'			=> 'ids'
		);

		$products = wc_get_products( $args );
		pewc_set_transient( 'pewc_product_ids' , $products );
...
function pewc_get_all_product_titles( $products ) {
	...
	$product_titles = pewc_get_transient( 'pewc_product_titles' );
	...
	if( ! $product_titles ) {
		...
		pewc_set_transient( 'pewc_product_titles', $product_titles );
...
/**
 * Update global group list if a global group has been trashed
 * @since 3.11.4
 */
function pewc_global_group_trashed( $post_id = '' ) {

	$global_order = get_option( 'pewc_global_group_order', '' );

	if ( empty( $global_order ) ) {
		return; // no need to proceed
	}

	$curr_global = explode( ',', $global_order );

	// Verify if is trashing multiple posts
	if ( isset( $_GET['post'] ) && is_array( $_GET['post'] ) ) {
		if ( did_action('wp_trash_post') > 1 ) {
			return; // this function gets called multiple times if an array, so let's try to prevent this
		}
		$to_remove = $_GET['post'];
	} else {
	...
	$new_global = array_diff( $curr_global, $to_remove );

	if ( count ( $new_global ) > 0 ) {
		$new_group_order = implode( ',', $new_global );
	}
	else {
		$new_group_order = '';
	}

	update_option( 'pewc_global_group_order', $new_group_order );
}
add_action( 'wp_trash_post', 'pewc_global_group_trashed', 10, 1 );
