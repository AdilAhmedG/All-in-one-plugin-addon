...
use Automattic\WooCommerce\Utilities\OrderUtil; // WC HPOS compliance
...
if( ! class_exists( 'PEWC_Product_Extra_Post_Type' ) ) {
	...
	class PEWC_Product_Extra_Post_Type {
		...
		public function init() {
			...
			add_filter( 'woocommerce_screen_ids', array( $this, 'add_aou_screen_ids' ), 10, 1);
		...
		public function enqueue_scripts( $hook ) {
			...
			if ( ! $is_post_type ) {
				// check if this is an order page using HPOS
				if ( function_exists( 'wc_get_page_screen_id' ) && 'woocommerce_page_wc-orders' === wc_get_page_screen_id( 'shop-order' ) ) {
					$is_post_type = true;
				}
			}
		...
		*/
					...
					<tbody id="the-list">
						...
						<tr>
							...
							<td>
								...
								<select multiple="multiple" class="pewc-rule-field wc-taxonomy-term-search" name="<?php echo esc_attr( $field['name'] ); ?>[categories][cats][]" data-name="" data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>" id="<?php echo esc_attr( $field['name'] ); ?>[categories][cats]">';
									<?php 
									if( ! empty( $field_value['categories']['cats'] ) ) { 
										foreach( $field_value['categories']['cats'] as $slug ) {
		...
		public function save_metabox_data( $post_id ) {
			...
			if( get_post_type( $post_id ) == 'pewc_group' ) {
				...
				return;
			} else if ( 'shop_order' === OrderUtil::get_order_type( $post_id ) ) {
				// WC HPOS compliance
					...
					if( $metabox['fields'] ) {
						foreach( $metabox['fields'] as $field ) {
							if( isset( $_POST[$field['name']] ) ) {
								...
								if ( $is_order ) {
									$order->update_meta_data( $field['ID'], $data ); // WC HPOS compliance
								} else {
									update_post_meta( $post_id, $field['ID'], $data );
								}
							} else {
								if ( $is_order ) {
									$order->delete_meta_data( $field['ID'] ); // WC HPOS compliance
								} else {
			...
			if ( $is_order ) {
				//$order->save(); // causes infinite loop when saving an order
			}
		}
		...
		public function save_group_metabox_data( $group_id ) {
			...
			if( isset( $_POST['group_class'] ) ) {
				$data = wp_kses_post( $_POST['group_class'] );
				update_post_meta( $group_id, 'group_class', $data );
			} else {
		...
		/**
		 * Add AOU screen IDs to WooCommerce's so that WC assets are loaded, e.g, for tipTip
		 * @since 3.11.9
		 */
		function add_aou_screen_ids( $screen_ids ) {
			$screen = get_current_screen();
			if ( ! empty( $screen->id ) && false !== strpos( $screen->id, '_page_global' ) ) {
				$screen_ids[] = $screen->id; // global add-ons, sometimes the screen id is translated so maybe we can catch them this way
			} else {
				$screen_ids[] = 'pewc_group'; // global group (edit as post type)
			}
