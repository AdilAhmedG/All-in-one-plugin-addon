...
<?php if( $item['child_products'] ) {
	...
	foreach( $item['child_products'] as $child_product_id ) {
		...
		$child_product_html .= sprintf(
			'<label for="%s" class="pewc-radio-list-label-wrapper"><input %s data-stock="%s" data-option-cost="%s" data-field-label="%s" type="radio" name="%s[]" id="%s" class="pewc-radio-form-field" value="%s" %s><span class="pewc-theme-element"></span></label><div class="pewc-radio-list-desc-wrapper">%s</div>',
				esc_attr( $radio_id ),
				esc_attr( $disabled ),
