	...
	$radio_buttons = '<ul class="pewc-checkbox-group-wrapper">';
	  ...
	  foreach( $item['field_options'] as $key=>$option_value ) {
	    ...
	    $radio_id = $id . '_' . strtolower( str_replace( ' ', '_', $key ) );
			...
			$option_attributes = apply_filters( 'pewc_radio_option_attributes', '', $item, $option_value, $key );
			$option_attribute_string = pewc_get_option_attribute_string( $option_attributes );

	    $radio = sprintf(
	      '<li><label class="%s" for="%s"><input data-option-cost="%s" type="radio" name="%s[]" id="%s" class="%s" data-option-percentage="%s" value="%s" %s %s>&nbsp;<span>%s</span><span class="pewc-theme-element"></span></label></li>',
				join( ' ', $label_classes ),
				...
				esc_attr( $option_percentage ),
	      		esc_attr( apply_filters( 'pewc_radio_option_value', $key, $item, $option_value ) ), // value
				esc_attr( $checked ),
				$option_attributes,
	      $label
	    );
